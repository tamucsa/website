...
export default function ImageModal({ imageId, className }: ImageModalProps) {
    return (
        <div className={`relative ${className}`}>
            ...
            <div className="flex flex-col my-10 mx-10 rounded-2xl ">
                <Image alt={`Officer ${imageId}`} src={`/officerImages/raw/${imageId}.JPG`} fill
                    className="rounded-2xl shadow-lg" objectFit="contain" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                />
            </div>
            {/* Buttons */} 
            <div className="absolute inset-3 mx-10 my-10 flex items-center justify-center">
